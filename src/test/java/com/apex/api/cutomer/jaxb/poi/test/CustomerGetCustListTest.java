...
public class CustomerGetCustListTest extends ApexBaseAPITest implements CustomerConstant {
	...
	HttpResponse response = (HttpResponse) ApexHttpUtil.sendAndReceiveGetMsg(BASE_USER_API_URL );
		...
		//Assert.assertEquals(statusCode, HTTP_STATUS_CODE_200);
		//Assert.assertEquals(msg, HTTP_STATUS_MESSAGE_OK);
		//Instead of asserting everytime we need to call validateStatusCodeReasonPhrase method
		ApexHttpValidator.validateStatusCodeReasonPhrase(response,HTTP_STATUS_CODE_200,HTTP_STATUS_MESSAGE_OK);
         /*
		 * //check for the tag name Assert.assertTrue(respString.contains("name"));
		 * Assert.assertTrue(respString.contains("id"));
		 */

		//check for the name
		CustomerList custmrLst=ApexJSONUtil. convertJsonToObjectList(respString);
	

	
	//	This code is for array of users
		ApexHttpValidator.validatorJsonKyValUsrsArray(name,  custmrLst,"first_name");
		ApexHttpValidator.validatorJsonKyValUsrsArray(id,  custmrLst,"id");
		ApexHttpValidator.validatorJsonKyValUsrsArray(email,  custmrLst,"email");
		//Assert.assertTrue(custmr.getData().get(0).containsKey("first_name"));
